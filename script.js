...
let forcedCaptureShownThisTurn = false; // Track if forced capture tutorial was shown this turn
...
function resetTutorialLessons() {
    tutorialLessonsShown = {
        ...
        basicMove: false,
        capture: false,
        forcedCapture: false,  // Keep this but we'll ignore it for forced captures
        doubleJump: false,
        kingPromotion: false,
        kingMovement: false,
        voiceCommands: false,
    ...
    forcedCaptureShownThisTurn = false;
}
...
function checkTutorialState() {
    ...
    // Reset forced capture shown flag for new turn
    forcedCaptureShownThisTurn = false;
    
    const possibleLessons = analyzeBoardForTutorial();
    
    for (const lesson of possibleLessons) {
        // For forced capture, always show it regardless of tutorialLessonsShown
        if (lesson.type === 'forcedCapture') {
            if (!forcedCaptureShownThisTurn) {
                showTutorialLesson(lesson);
                forcedCaptureShownThisTurn = true;
            }
            break;
        }
        // For other lessons, check if they've been shown
        else if (!tutorialLessonsShown[lesson.type]) {
            showTutorialLesson(lesson);
            break;
        }
    }
...
function analyzeBoardForTutorial() {
    ...
    // ALWAYS check for forced capture first - highest priority
    if (mustCapture && !forcedCaptureShownThisTurn) {
        lessons.push({
            type: 'forcedCapture',
            piece: capturablePieces[0],
            move: capturablePieces[0].captures[0]
        });
        return lessons; // Return immediately - forced capture takes priority
    }
...
function showTutorialLesson(lesson) {
    ...
    switch(lesson.type) {
        ...
        case 'forcedCapture':
            // Dynamic message that shows every time
            const pieceLabel = gameBoard[lesson.piece.row][lesson.piece.col].label;
            title = 'Mandatory Capture!';
            text = `You MUST capture when possible! ${pieceLabel} can capture an opponent's piece. This is a mandatory rule in checkers - you cannot make any other move when a capture is available.`;
...
function checkForForcedMoves() {
    ...
    // If tutorial mode and must capture for player 1, trigger tutorial check
    if (tutorialMode && mustCapture && currentPlayer === 1 && !forcedCaptureShownThisTurn) {
...
function makeMove(row, col, isAIMove = false) {
    ...
    if (validMove) {
        ...
        if (!checkGameEnd()) {
            ...
            // Reset forced capture shown flag when turn changes
            forcedCaptureShownThisTurn = false;
            checkForForcedMoves();
            updateTurnDisplay();
            createBoard();
