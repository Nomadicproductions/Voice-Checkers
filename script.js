...
let isAIMoving = false; // Add flag to track AI movement
...
function getPossibleMoves(row, col) {
    ...
    directions.forEach(([dRow, dCol]) => {
        ...
        if (!gameBoard[newRow][newCol]) {
            // Only add non-jump moves if we're not in a must-capture situation
...
// Make Move - FIXED for AI double jumps
    ...
    if (validMove) {
        ...
        if (madeCapture) {
            ...
            if (additionalCaptures.length > 0) {
                selectedPiece = { row, col };
                clearHighlights();
                checkForForcedMoves(); // Re-check for forced moves
                createBoard();
                ...
                if (isAIMove && gameMode === 'ai' && currentPlayer === 2) {
                    setTimeout(() => {
                        selectPiece(row, col);
                        setTimeout(() => {
                            // Select the best additional capture
                            const bestCapture = additionalCaptures[0]; // Could enhance this with better selection
                            makeMove(bestCapture.row, bestCapture.col, true);
                        }, 800); // Delay so player can see what's happening
                    }, 800);
                } else {
                    selectPiece(row, col);
                    updateTurnDisplay();
        ...
        if (!checkGameEnd()) {
            ...
            // Check again after turn switch if new player has no moves
            checkGameEnd();
...
// AI Implementation - FIXED
function makeAIMove() {
    if (gameEnded) return;
    
    isAIMoving = true; // Set flag to prevent manual interference
    checkForForcedMoves();
    ...
    if (selectedMove) {
        ...
        setTimeout(() => {
            makeMove(selectedMove.to.row, selectedMove.to.col, true); // Pass true for AI move
            isAIMoving = false; // Reset flag after move
        }, 500);
    } else {
        isAIMoving = false;
    }
...
// Check Game End - FIXED to detect no moves available
function checkGameEnd() {
    ...
    // 2. Current player has no valid moves (blocked/stuck)
    if (player1Pieces === 0) {
