...
// Loading Screen and Intro Sequence
window.addEventListener('load', function() {
    ...
    preloadImages(() => {
        // After images are loaded, complete loading screen
        setTimeout(() => {
...
// Play Intro Sequence - Updated with 3.5 second display time
...
function startTutorial() {
    ...
    showTutorialMessage(
        'Welcome to Checkers!',
        tutorialControlMode === 'voice' ? 
            'You\'ve chosen to play with voice commands. I\'ll guide you through the game. Say "move [piece] to [square]" to make moves. The microphone will activate automatically.' :
            'You\'ve chosen to play with touch/click controls. I\'ll guide you through the game. Click on pieces to select them, then click where you want to move.',
        null,
        true
    );
}
...
function showTutorialLesson(lesson) {
    ...
    showTutorialMessage(title, text, lesson);
...
function showTutorialMessage(title, text, lesson = null, showButton = false) {
    ...
    if (lesson && lesson.piece) {
        const square = document.querySelector(`[data-row="${lesson.piece.row}"][data-col="${lesson.piece.col}"]`);
        if (square) {
            const rect = square.getBoundingClientRect();
            const boxWidth = 350;
            
            // Position box near the piece
            box.style.left = Math.min(Math.max(10, rect.left - boxWidth/2 + rect.width/2), window.innerWidth - boxWidth - 10) + 'px';
            box.style.top = (rect.top > window.innerHeight/2 ? rect.top - 200 : rect.bottom + 20) + 'px';
        }
    } else {
        // Center the box
        box.style.left = '50%';
    ...
    gameModeRadios.forEach(radio => {
        radio.addEventListener('change', function() {
            ...
            if (gameMode === 'ai') {
                ...
                if (!tutorialLessonsShown.basicMove) {
                    setTimeout(() => {
...
function makeMove(row, col, isAIMove = false) {
    ...
    if (validMove) {
        ...
        if (validMove.isJump) {
            ...
            // Tutorial: Check for double jump lesson
            if (tutorialMode && !tutorialLessonsShown.doubleJump) {
        ...
        if (!piece.isKing) {
            if ((piece.player === 1 && row === 0) || (piece.player === 2 && row === 7)) {
                ...
                // Tutorial: King movement lesson
                if (tutorialMode && !tutorialLessonsShown.kingMovement && piece.player === 1) {
