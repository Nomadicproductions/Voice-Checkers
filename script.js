...
function preloadImages(callback) {
    ...
    // Absolute timeout - proceed after 5 seconds no matter what
    setTimeout(() => {
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    preloadImages(() => {
        console.log('Preloading complete - hiding loading screen');
        ...
        setTimeout(() => {
            ...
            // Try to play intro sequence
            try {
                playIntroSequence();
            } catch (err) {
                console.error('Intro sequence failed:', err);
                // If intro fails, go straight to start screen
                showStartScreen();
            }
        }, 1000);
    });
    
    // ULTIMATE FAILSAFE - If nothing happens after 8 seconds, force start
    setTimeout(() => {
        const loadingScreen = document.getElementById('loading-screen');
...
document.getElementById('play-again-btn').onclick = function() {
    ...
    document.getElementById('game-container').style.display = 'flex';
    // Clear tutorial mode when playing again after game ends
    tutorialMode = false;
    restartGame();
};

// Tutorial Prompt Handlers - FIXED to ensure tutorial mode stays active
document.getElementById('tutorial-yes-btn').onclick = function() {
    ...
    document.getElementById('tutorial-touch-btn').onclick = function() {
        console.log('Tutorial Mode Activated: Touch Control');
        tutorialMode = true;  // Ensure tutorial mode is set
        tutorialControlMode = 'touch';
        gameMode = 'ai';  // Ensure AI mode is set
        aiDifficulty = 'easy';
        resetTutorialLessons();
        document.getElementById('tutorial-prompt').style.display = 'none';
        
        // Initialize game with tutorial mode
        initializeBoard();
        createBoard();
        checkForForcedMoves();
        updateTurnDisplay();
    ...
    document.getElementById('tutorial-voice-btn').onclick = function() {
        console.log('Tutorial Mode Activated: Voice Control');
        tutorialMode = true;  // Ensure tutorial mode is set
...
document.getElementById('tutorial-no-btn').onclick = function() {
    console.log('Tutorial Mode Declined');
    tutorialMode = false;
    document.getElementById('tutorial-prompt').style.display = 'none';
};
...
function startTutorial() {
    console.log('Starting Tutorial - tutorialMode:', tutorialMode);
    tutorialStep = 0;
...
function checkTutorialState() {
    console.log('Checking tutorial state - tutorialMode:', tutorialMode, 'currentPlayer:', currentPlayer, 'mustCapture:', mustCapture);
...
function showForcedCaptureTutorial() {
    console.log('showForcedCaptureTutorial called - mustCapture:', mustCapture, 'capturablePieces:', capturablePieces.length);
...
// Settings Controls - COMPLETELY FIXED to never disable tutorial during active session
document.addEventListener('DOMContentLoaded', function () {
    ...
    gameModeRadios.forEach(radio => {
        radio.addEventListener('change', function() {
            ...
            const wasInTutorial = tutorialMode;
            ...
            if (gameMode === 'ai') {
                ...
                // Only show tutorial prompt if not already in tutorial and haven't seen it before
                if (!wasInTutorial && !tutorialLessonsShown.gameObjective) {
                    setTimeout(() => {
                        document.getElementById('tutorial-prompt').style.display = 'flex';
                    }, 500);
                } else if (!wasInTutorial) {
                    // If not in tutorial and have seen it before, just restart the game
                    restartGame();
                }
                // If was in tutorial, don't restart - keep tutorial active
            } else {
                aiDifficultyContainer.style.display = 'none';
                tutorialModeContainer.style.display = 'none';
                // Only end tutorial if switching away from AI mode
                if (wasInTutorial) {
                    tutorialMode = false;
                    endTutorial();
                }
    ...
    aiDifficultySelect.addEventListener('change', function(event) {
        event.stopPropagation();
        const oldDifficulty = aiDifficulty;
        aiDifficulty = this.value;
        console.log('AI Difficulty changed to:', aiDifficulty);
        
        // Only restart if not in tutorial mode and difficulty actually changed
        if (gameMode === 'ai' && !tutorialMode && oldDifficulty !== aiDifficulty) {
            restartGame();
        }
    });
    ...
    document.getElementById('reset-game-btn').addEventListener('click', function(event) {
        event.stopPropagation();
        // Store tutorial state before restart
        const wasInTutorial = tutorialMode;
        restartGame();
        // If was in tutorial, restart it
        if (wasInTutorial) {
            tutorialMode = true;
...
// Check for Forced Moves (captures) - ENHANCED with debugging
function checkForForcedMoves() {
    ...
    // Debug logging
    console.log('Force capture check - mustCapture:', mustCapture, 'player:', currentPlayer, 'tutorialMode:', tutorialMode, 'isAIMoving:', isAIMoving);
    ...
    if (tutorialMode && mustCapture && currentPlayer === 1 && !isAIMoving) {
        console.log('Triggering tutorial check for forced capture');
...
// Make Move - FIXED to reset forced capture state
...
// Restart Game - FIXED to properly handle tutorial mode
function restartGame() {
    // Store tutorial state before resetting
    const preserveTutorial = tutorialMode;
    
    console.log('Restarting game - preserveTutorial:', preserveTutorial);
    ...
    // Only clear tutorial if explicitly ending it
    if (!preserveTutorial) {
        tutorialMode = false;
        document.getElementById('tutorial-overlay').style.display = 'none';
    } else {
        // Keep tutorial active
        tutorialMode = true;
        clearTutorialHighlights();
        document.getElementById('tutorial-overlay').style.display = 'none';
    }
    ...
    // If tutorial mode is active, check tutorial state after restart
    if (tutorialMode) {
        setTimeout(() => checkTutorialState(), 100);
    }
}
...
function executeVoiceMove(pieceLabel, targetSquare) {
    ...
    const validMove = validMoves.find(move => move.row === targetCoords.row && move.col === targetCo
