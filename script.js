...
let tutorialLessonsShown = {
    ...
    basicMove: false,
    capture: false,
    forcedCapture: false,  // We'll ignore this and always show forced captures
    doubleJump: false,
    kingPromotion: false,
    kingMovement: false,
    voiceCommands: false,
...
let forcedCaptureTutorialActive = false; // Track if forced capture tutorial is currently showing
...
function resetTutorialLessons() {
    ...
    forcedCaptureTutorialActive = false;
}
...
// Check Tutorial State - COMPLETELY REWRITTEN
function checkTutorialState() {
    if (!tutorialMode || currentPlayer !== 1 || isAIMoving) return;
    
    console.log('Checking tutorial state - mustCapture:', mustCapture, 'forcedCaptureTutorialActive:', forcedCaptureTutorialActive);
    
    // ALWAYS show forced capture tutorial when there's a forced capture
    if (mustCapture && !forcedCaptureTutorialActive) {
        console.log('Showing forced capture tutorial');
        showForcedCaptureTutorial();
        return;
    }
    
    // Check for other tutorial opportunities
...
// Show Forced Capture Tutorial - DEDICATED FUNCTION
function showForcedCaptureTutorial() {
    if (!mustCapture || capturablePieces.length === 0) return;
    
    forcedCaptureTutorialActive = true;
    
    // Get all pieces that can capture with their labels
    const captureDetails = capturablePieces.map(p => {
        const piece = gameBoard[p.row][p.col];
        return piece ? piece.label : '';
    }).filter(label => label);
    
    // Use the first capturable piece for highlighting
    const firstCapture = capturablePieces[0];
    const piece = gameBoard[firstCapture.row][firstCapture.col];
    
    let message;
    if (capturablePieces.length > 1) {
        message = `MANDATORY CAPTURE! You have ${capturablePieces.length} pieces that can capture: ${captureDetails.join(', ')}. You MUST use one of them to capture an opponent's piece. This is a fundamental rule in checkers!`;
    } else {
        message = `MANDATORY CAPTURE! ${piece.label} must capture an opponent's piece. When a capture is available, you cannot make any other move. This rule is always enforced!`;
    }
    
    // Don't force a specific piece in tutorial if multiple can capture
    if (capturablePieces.length === 1) {
...
function initializeGame() {
    ...
    // Immediately check tutorial state after initialization
    if (tutorialMode) {
        setTimeout(() => checkTutorialState(), 200);
    }
}
...
// Check for Forced Moves - IMPROVED
function checkForForcedMoves() {
    ...
    console.log('checkForForcedMoves - mustCapture:', mustCapture, 'currentPlayer:', currentPlayer, 'tutorialMode:', tutorialMode);
    
    // IMMEDIATE tutorial check if forced capture for player 1
    if (tutorialMode && mustCapture && currentPlayer === 1 && !isAIMoving && !forcedCaptureTutorialActive) {
        console.log('Triggering forced capture tutorial from checkForForcedMoves');
        // Use immediate execution instead of timeout
        checkTutorialState();
    }
}
        ...
        for (let col = 0; col < 8; col++) {
            ...
            if (piece) {
                ...
                pieceElement.onclick = (e) => {
                    ...
                    // Allow any capturable piece in forced capture
...
function selectPiece(row, col) {
    ...
    // Clear forced capture tutorial when piece is selected
    if (forcedCaptureTutorialActive && mustCapture) {
        forcedCaptureTutorialActive = false;
        document.getElementById('tutorial-overlay').style.display = 'none';
    ...
    if (validMove) {
        ...
        if (madeCapture) {
            ...
            if (additionalCaptures.length > 0) {
                selectedPiece = { row, col };
                clearHighlights();
                // Reset forced capture tutorial state for double jump
                forcedCaptureTutorialActive = false;
                checkForForcedMoves();
                createBoard();
        ...
        if (!checkGameEnd()) {
            ...
            // Reset forced capture tutorial state when turn changes
            forcedCaptureTutorialActive = false;
            checkForForcedMoves();
            updateTurnDisplay();
            createBoard();
