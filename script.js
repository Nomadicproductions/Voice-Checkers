...
// Get Filtered Board Coordinate - FIXED
function getFilteredBoardCoordinate(row, col) {
    const letters = 'abcdefgh';
    // Map array index to display number: 0->3, 1->4, 2->5, 3->6, 4->7, 5->8, 6->9, 7->10
    const validNumbers = [3, 4, 5, 6, 7, 8, 9, 10];
    const colNumber = validNumbers[col];
    
    if (colNumber === undefined || colNumber.toString().includes('2')) {
        return null;
    }
...
function getPossibleMoves(row, col) {
    ...
    directions.forEach(([dRow, dCol]) => {
        ...
        // Check if this is a valid dark square
        if ((newRow + newCol) % 2 === 0) return; // Light squares are not valid
        ...
        if (!gameBoard[newRow][newCol]) {
            ...
            if (jumpRow >= 0 && jumpRow < 8 && jumpCol >= 0 && jumpCol < 8) {
                // Check if jump destination is a valid dark square
                if ((jumpRow + jumpCol) % 2 === 1 && !gameBoard[jumpRow][jumpCol]) {
                    moves.push({
                        row: jumpRow,
                        col: jumpCol,
                        isJump: true,
...
function parseVoiceCommand(command) {
    try {
        ...
        // Voice move commands
        const movePatterns = [
            /move ([ln]\d+) to ([a-h](?:3|4|5|6|7|8|9|10))/i,
            /([ln]\d+) to ([a-h](?:3|4|5|6|7|8|9|10))/i,
            /piece ([ln]\d+) to ([a-h](?:3|4|5|6|7|8|9|10))/i
        ];
...
// Square to Coordinates - COMPLETELY FIXED
function squareToCoordinates(square) {
    if (square.length < 2 || square.length > 3) return null;
    const letter = square[0];
    const numberStr = square.slice(1);
    
    if (letter < 'a' || letter > 'h') return null;
    
    const number = parseInt(numberStr);
    // Check if number contains '2' or is out of valid range (3-10, excluding numbers with '2')
    if (isNaN(number) || number < 3 || number > 10 || numberStr.includes('2')) {
        return null;
    }
    
    const row = letter.charCodeAt(0) - 'a'.charCodeAt(0);
    
    // Map the filtered coordinate system (3,4,5,6,7,8,9,10) to array indices (0-7)
    // The valid numbers are: 3->0, 4->1, 5->2, 6->3, 7->4, 8->5, 9->6, 10->7
    let col;
    const validNumbers = [3, 4, 5, 6, 7, 8, 9, 10];
    const colIndex = validNumbers.indexOf(number);
    ...
    // Also verify this is a valid dark square (only dark squares are playable)
    if ((row + col) % 2 === 0) {
        return null; // This would be a light square, which is invalid
    }
