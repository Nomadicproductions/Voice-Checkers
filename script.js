...
// Settings Controls - FIXED: Removed restartGame() calls that were disabling tutorial
document.addEventListener('DOMContentLoaded', function () {
    ...
    gameModeRadios.forEach(radio => {
        radio.addEventListener('change', function() {
            ...
            if (gameMode === 'ai') {
                ...
                // FIXED: Don't call restartGame() here if tutorial is active
                if (!tutorialMode) {
                    restartGame();
                }
            } else {
                ...
                // FIXED: Only end tutorial and restart if actually in tutorial mode
                if (tutorialMode) {
                    tutorialMode = false;
                    endTutorial();
                }
    ...
    aiDifficultySelect.addEventListener('change', function(event) {
        ...
        // FIXED: Don't restart game if in tutorial mode
        if (gameMode === 'ai' && !tutorialMode) {
            restartGame();
        }
    });
...
// Restart Game - FIXED: Preserve tutorial mode during restart
function restartGame() {
    ...
    // Only end tutorial if explicitly not in tutorial mode
    if (!preserveTutorial) {
        if (tutorialMode) {
            endTutorial();
        }
    } else {
        // Keep tutorial mode active but clear highlights
        clearTutorialHighlights();
        document.getElementById('tutorial-overlay').style.display = 'none';
    }
