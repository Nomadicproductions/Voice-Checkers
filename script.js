...
let aiCapturedFirst = false;  // Track if AI captured first in tutorial
...
function analyzeBoardForTutorial() {
    ...
    // Check for capture opportunity - but only if AI hasn't captured first
    if (!tutorialLessonsShown.capture && !aiCapturedFirst) {
...
function makeMove(row, col, isAIMove = false) {
    ...
    if (validMove) {
        ...
        if (validMove.isJump) {
            ...
            // Track if AI captured first in tutorial
            if (tutorialMode && isAIMove && !tutorialLessonsShown.capture) {
                aiCapturedFirst = true;
            }
            ...
            if (tutorialMode && !tutorialLessonsShown.doubleJump && currentPlayer === 1) {
